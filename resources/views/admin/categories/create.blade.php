...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-future text-3xl font-semibold text-pip-green">Create New Category</h1>
        ...
        <form action="{{ route('admin.categories.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="icon" id="icon" class="form-input" value="{{ old('icon') }}" placeholder="gg-book">
                    @error('icon')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end mt-6">
                <button type="submit" class="btn btn-primary flex items-center gap-2">
                    <i class="gg-add"></i>
                    <span class="terminal-text">Create Category</span>
                </button>
            </div>
        </form>
    </div>
