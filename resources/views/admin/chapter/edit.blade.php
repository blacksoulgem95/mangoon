{{-- resources/views/admin/chapter/edit.blade.php --}}
@extends('layouts.app')
...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-future text-3xl font-semibold text-pip-green">Edit Chapter for "{{ $manga->title }}"</h1>
        <div class="space-x-2">
    ...
    <form action="{{ route('admin.manga.chapters.update', [$manga, $chapter]) }}" method="POST" enctype="multipart/form-data" class="panel p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" id="slug" name="slug" value="{{ old('slug', $chapter->slug) }}"
            ...
            <div>
                ...
                <input type="text" id="chapter_number" name="chapter_number" value="{{ old('chapter_number', $chapter->chapter_number) }}"
                    class="w-full p-2 bg-bg-screen border border-border rounded text-mono"
            ...
            <div>
                ...
                <input type="number" id="volume_number" name="volume_number" value="{{ old('volume_number', $chapter->volume_number) }}"
                    class="w-full p-2 bg-bg-screen border border-border rounded text-mono"
                    placeholder="e.g., 1, 2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <textarea id="notes" name="notes"
                    class="w-full p-2 bg-bg-screen border border-border rounded text-mono"
                    rows="4" placeholder="Any additional info">{{ old('notes', $chapter->notes) }}</textarea>
        ...
        {{-- CBZ Upload --}}
        <div>
            <label for="cbz_file" class="block font-medium text-future text-pip-green mb-1">CBZ File (optional – replace existing)</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $chapter->sort_order) }}"
                    class="w-full p-2 bg-bg-screen border border-border rounded text-mono">
            ...
            <div>
                ...
                <textarea id="metadata" name="metadata"
                    class="w-full p-2 bg-bg-screen border border-border rounded text-mono"
                    rows="4" placeholder='{"key":"value"}'>{{ old('metadata', $chapter->metadata ? json_encode($chapter->metadata, JSON_PRETTY_PRINT) : '') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $chapter->is_active) ? 'checked' : '' }} class="mr-2">
                <span>Make chapter visible in the library</span>
            </div>

            <div>
                <label class="block font-medium text-future text-pip-green mb-1">Premium</label>
                <input type="checkbox" id="is_premium" name="is_premium" value="1" {{ old('is_premium', $chapter->is_premium) ? 'checked' : '' }} class="mr-2">
        ...
        <div class="flex justify-end space-x-4 mt-6">
            ...
            <button type="submit" class="btn btn-primary">
                <span class="terminal-text">Save Chapter</span>
            </button>
        </div>
    </form>
</div>
