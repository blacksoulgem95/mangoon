{{-- resources/views/admin/chapters/show.blade.php --}}
@extends('layouts.app')
...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-future text-3xl font-semibold text-pip-green">{{ $chapter->displayName() }} - {{ $chapter->manga->title }}</h1>

        <div class="space-x-2">
            <a href="{{ route('admin.chapters.index') }}" class="btn btn-secondary">
                <span class="terminal-text">Back to Chapters</span>
            </a>
            <a href="{{ route('admin.manga.chapters.edit', [$chapter->manga, $chapter]) }}" class="btn btn-primary">
    ...
    <div class="panel p-6 space-y-6">
        ...
        @if($chapter->page_count > 0)
            <div class="mt-8">
                <h2 class="text-future font-semibold text-pip-green mb-4">Pages ({{ $chapter->page_count }})</h2>
                <div class="grid grid-cols-4 gap-4">
                    @for($i = 1; $i <= $chapter->page_count; $i++)
                        <a href="{{ route('chapter.page', [$chapter->manga, $chapter, $i]) }}" class="block w-full h-32 bg-bg-screen rounded hover:shadow-md transition-all">
                            <span class="text-center text-mono text-sm mt-12 block">{{ $i }}</span>
                        </a>
                    @endfor
                </div>
            </div>
