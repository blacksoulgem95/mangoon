...
@section('title', ' • Admin: Edit Library')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-future text-3xl font-semibold text-pip-green">Edit Library - {{ $library->name }}</h1>
        ...
        <form action="{{ route('admin.libraries.update', $library) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" class="form-input" value="{{ old('name', $library->name) }}" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" class="form-input" value="{{ old('slug', $library->slug) }}" required>
                    @error('slug')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="icon" id="icon" class="form-input" value="{{ old('icon', $library->icon) }}" placeholder="gg-folder">
                    @error('icon')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="color" id="color" class="form-input" value="{{ old('color', $library->color) }}" placeholder="#000000">
                    @error('color')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" class="form-input" value="{{ old('sort_order', $library->sort_order) }}">
                    @error('sort_order')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    <label for="is_active" class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="is_active" id="is_active" class="form-checkbox" {{ old('is_active', $library->is_active) ? 'checked' : '' }}>
                ...
                <div class="md:col-span-2">
                    <label for="is_public" class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="is_public" id="is_public" class="form-checkbox" {{ old('is_public', $library->is_public) ? 'checked' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" class="form-textarea">{{ old('description', $library->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end mt-6">
                <button type="submit" class="btn btn-primary flex items-center gap-2">
                    <i class="gg-check"></i>
                    <span class="terminal-text">Update Library</span>
                </button>
            </div>
        </form>
    </div>
