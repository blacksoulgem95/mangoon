...
@section('title', ' • Admin: Tags List')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-future text-3xl font-semibold text-pip-green">Tags Management</h1>
        ...
        <div class="space-x-2 flex">
            ...
            <a href="{{ route('admin.tags.create') }}" class="btn btn-primary flex items-center gap-2">
                <i class="gg-add"></i>
                <span class="terminal-text">New Tag</span>
            </a>
        </div>
    </div>

    {{-- Search & Filters --}}
    <form method="GET" action="{{ route('admin.tags.index') }}" class="flex flex-wrap gap-3 mb-6">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name…" class="p-2 bg-bg-screen border border-border rounded text-mono w-full sm:w-64">
        ...
        <table class="w-full">
            <thead>
                <tr>
                    ...
                    <th class="p-3 text-future font-semibold text-pip-green">Color</th>
            ...
            <tbody>
                @forelse($tags as $tag)
                    <tr class="border-b border-border hover:bg-bg-panel">
                        ...
                        <td class="p-3">{{ $tag->slug }}</td>
                        <td class="p-3">
                            <span class="inline-block w-6 h-6 rounded-full" style="background-color: {{ $tag->color ?? '#CCCCCC' }}"></span>
                            {{ $tag->color ?? 'N/A' }}
                        </td>
                        <td class="p-3 text-center">{{ $tag->sort_order }}</td>
                        ...
                        <td class="p-3 space-x-2">
                            <a href="{{ route('admin.tags.edit', $tag) }}" class="btn btn-secondary btn-sm inline-flex items-center gap-2">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="p-4 text-center text-text-primary">No tags found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="mt-6 flex justify-center">
        {{ $tags->withQueryString()->links('vendor.pagination.default') }}
    </div>
</div>
@endsection
